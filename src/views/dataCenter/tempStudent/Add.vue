        ...
        >
            <Form inline ref="obj" :model="obj" :rules="rules" label-position="top" class="modal-form">
                <FormItem label="学校名称" prop="dep_id">
                    <i-select style="width:160px" v-model="obj.dep_id" filterable @on-change="schoolChange"
                              :disabled="flag">
                ...
                <FormItem label="班级" prop="class_no">
                    <i-select style="width:160px" v-model="obj.class_no" :disabled="flag" @on-change="classChange">
                        <i-option v-for="item in classList" :value="item.id" :key="item.id">{{item.class_no}}</i-option>
                    </i-select>
                </FormItem>
                <FormItem label="学籍号" prop="code">
                    <i-input style="width:170px" :maxlength="19" :disabled="flag" v-model.trim="obj.code"
                             placeholder=""></i-input>
                </FormItem>
                <FormItem label="学生姓名" prop="name">
                    <i-input style="width:150px" :maxlength="20" :disabled="flag" v-model.trim="obj.name"
                             placeholder=""></i-input>
                </FormItem>
                ...
                <FormItem label="民族" prop="nation">
                    <i-input style="width:160px" :maxlength="20" :disabled="flag" v-model.trim="obj.nation"
                             placeholder=""></i-input>
                </FormItem>
                <FormItem label="户籍类型" prop="household_type">
                    <i-select style="width:170px" v-model="obj.household_type" :disabled="flag">
                ...
                <FormItem v-if="card_flag" label="证件号" prop="card_num">
                    <i-input style="width:160px" v-model.trim="obj.card_num" key="1"
                             :maxlength="18" :disabled="flag" placeholder=" " @on-blur="idBlur"></i-input>
                </FormItem>
                <FormItem v-if="!card_flag" label="证件号" prop="card_num1">
                    <i-input style="width:160px" :disabled="flag" v-model.trim="obj.card_num1" key="2"
                             :maxlength="9" placeholder=" "></i-input>
                </FormItem>
                <FormItem label="生日" prop="birthday">
                    <DatePicker type="date" format="yyyy/MM/dd" v-model="obj.birthday" :disabled="card_flag||flag"
                                style="width: 170px!important;"></DatePicker>
                </FormItem>

                <FormItem label="第一监护人" prop="guardian1">
                    <i-input style="width:150px" :maxlength="20" :disabled="flag" v-model.trim="obj.guardian1"
                             placeholder=" "></i-input>
                </FormItem>
                <FormItem label="第一监护人手机号" prop="guardian1_phone">
                    <i-input style="width:150px" :maxlength="11" :disabled="flag" v-model.trim="obj.guardian1_phone"
                             placeholder=" "></i-input>
                </FormItem>
                <FormItem label="第一监护人工作类型">
                    <i-input style="width:160px" :maxlength="20" :disabled="flag" v-model.trim="obj.guardian1_type"
                             placeholder=" "></i-input>
                </FormItem>
                <FormItem label="第二监护人" prop="guardian2">
                    <i-input style="width:170px" :maxlength="20" :disabled="flag" v-model.trim="obj.guardian2"
                             placeholder=" "></i-input>
                </FormItem>
                <FormItem label="第二监护人手机号" prop="guardian2_phone">
                    <i-input style="width:150px" :maxlength="11" :disabled="flag" v-model.trim="obj.guardian2_phone"
                             placeholder=" "></i-input>
                </FormItem>
                <FormItem label="第二监护人工作类型">
                    <i-input style="width:150px" :maxlength="20" :disabled="flag" v-model.trim="obj.guardian2_type"
                             placeholder=" "></i-input>
                </FormItem>
                ...
                <FormItem label="所属计划" prop="plan_id">
                    <i-select style="width:150px" v-model="obj.plan_id" :disabled="flag" @on-change="pick">
                        <i-option v-for="item in planList" :value="item.id" :key="item.id">{{item.code}}</i-option>
                    </i-select>
                </FormItem>
            </Form>
            <div slot="footer">
...
<script>
    ...
    import {getAllSchool, getTempStudentList} from '@/api/basicData';
        ...
        data() {
            return {
                ...
                rules: {
                    ...
                    card_num1: [
                        {required: true, message: ' 护照号', trigger: 'blur'},
        ...
        methods: {
            idBlur(event) {
                ...
                if (value.length === 18) {
                    ...
                    this.$set(this.obj, 'birthday', new Date(y + '/' + m + '/' + d));
            ...
            cardChange() {
                if (this.obj.card_type == 1) {
                    ...
                    this.rules.birthday = [];
                    this.obj.birthday = '';
                    this.$refs ['obj'].fields.forEach(function (e) {
                        if (e.prop == 'birthday') {
                            e.resetField()
                        }
                    })
                    ...
                    this.rules.birthday = [{required: true, type: 'date', message: '请选择生日', trigger: 'change'}]
                    this.$refs ['obj'].fields.forEach(function (e) {
                        if (e.prop == 'card_num') {
                            e.resetField()
                        }
            ...
            radioChange() {
                if (this.disease.past_medical === '是') {
                    ...
                    this.rules.past_time = [{required: true, type: 'date', message: '结核病史日期不能为空', trigger: 'change'}]
                } else {
                    this.showOrHide = false;
                    this.rules.past_time = [];
                    this.disease.past_time = '';
                    this.$refs ['obj'].fields.forEach(function (e) {
                        if (e.prop == 'past_time') {
                            e.resetField()
                        }
            ...
            pick(e) {
                this.planList.map((item, i) => {
                    if (item.id == e) {
                        this.obj.plan_code = item.code; //传值
                        this.obj.plan_id = item.id;//传值
                    }
                });
            },
            ...
            renderDate(h, params, filed) {
                if (this.flag) {
                    return h('span', null, dateFormatFromString(params.row[filed]));
                }
                return h('DatePicker', {
                    props: {
                        type: 'date',
            ...
            schoolChange(value) {
                this.schoolList.map((item, i) => {
                    if (item.id == value) {
                        this.obj.school_name = item.name; //学校
                        this.obj.dept_id = item.id;//传值
                    }
                });
                this.obj.grade_no = '';
                if (value === -1) {
            ...
            gradeChange(value) {
                this.gradeList.map((item, i) => {
                    if (item.id == value) {
                        this.obj.grade_name = item.value;
                        this.obj.grade_no = item.id;
                    }
                });
                this.obj.class_no = '';
                this.$ajax({
            ...
            classChange(value) {
                this.classList.map((item, i) => {
                    if (item.id == value) {
                        this.obj.class_name = item.class_no;
                        this.obj.class_no = item.id;
                    }
                });
            },
            search() {
                Promise.all([getAllSchool(), getTempStudentList(this.id)]).then(value => {
                    ...
                    if (data.code === 200) {
                        ...
                        this.obj.stulist_id = data.data.stulist_id;
                        this.obj.birthday = data.data.birthday;
                        this.obj.year = data.data.year;
                        this.obj.term = data.data.term;
                        this.obj.grade_no = data.data.grade_no;
                        this.obj.nation = data.data.nation;
                        this.obj.guardian1 = data.data.guardian1;
                        this.obj.guardian1_phone = data.data.guardian1_phone;
                        this.obj.guardian1_type = data.data.guardian1_type;
                        this.obj.guardian2 = data.data.guardian2;
                        this.obj.guardian2_phone = data.data.guardian2_phone;
                        this.obj.guardian2_type = data.data.guardian2_type;

                        this.gradeChange(data.data.grade_no);
                        this.getPlanList(data.data.plan_id);
                        ...
                        if (data.data.card_type === 1) {
                            ...
                            this.rules.birthday = [{required: false, type: 'date', message: '请选择生日', trigger: 'change'}]
                            this.card_flag = false;
                        } else {
                            this.obj.card_num = data.data.card_num;
                            this.card_flag = true;
                        ...
                        this.obj.guardian2_type = data.data.guardian2_type;
                        this.obj.guardian2_phone = data.data.guardian2_phone;
                        this.obj.plan_id = data.data.plan_id;
                        this.obj.plan_code = data.data.plan_code;

                        this.disease = {
                            past_medical: data.data.past_medical === 2 ? '是' : '否',
                            past_time: data.data.past_medical === 2 ? new Date(data.data.pasttime) : null,
                        ...
                        this.tableData1 = [
                            ...
                            {
                                type: '时间',
                                hepatitistime: data.data.hepatitistime ? new Date(data.data.hepatitistime) : '',
                                tbtime: data.data.tbtime ? new Date(data.data.tbtime) : '',
                                touchtbtime: data.data.touchtbtime ? new Date(data.data.touchtbtime) : '',
                                chdtime: data.data.chdtime ? new Date(data.data.chdtime) : '',
                                lgatime: data.data.lgatime ? new Date(data.data.lgatime) : '',
                                rheumatismtime: data.data.rheumatismtime ? new Date(data.data.rheumatismtime) : '',
                                asthmatime: data.data.asthmatime ? new Date(data.data.asthmatime) : ''
                            }
                        ];
                        this.tableData2 = [
                            {
            ...
            confirms() {
                ...
                this.$refs['obj'].validate(valid => {
                    ...
                    let data = {
                        ...
                        stulist_id: this.obj.stulist_id,
                        year: this.obj.year,
                        term: this.obj.term,
                        ...
                        school_name: this.obj.school_name,
                        grade_name: this.obj.grade_name,
                        class_name: this.obj.class_name
                    ...
                    if (this.tableData1[0].touch_tb === 2) {
                        if (!this.tableData1[1].touchtbtime) {
                            this.$Message.error('请填写有无肺结核密切接触史时间');
                            return false;
                        }
                        console.log(this.tableData1[1].touchtbtime);
